...
def analyze_frame(frame, model, depth_estimator, output_folder, original_name, debug=False, debug_folder="debug_images"):
    ...
    pins = []
    status_message = "No red pin detected"

    if result.masks is not None:
        for idx, (mask, cls) in enumerate(zip(result.masks, result.boxes.cls)):
            np_mask = mask.data.cpu().numpy()[0].astype(np.uint8)
            final_mask = cv2.bitwise_or(final_mask, np_mask)
            if cls == 1:  # Assuming class 1 is for pins
                color = analyze_mask_color(np_mask, frame, debug=True, debug_folder="debug_images")
                depth = analyze_depth(img_depth, np_mask)
                pins.append({"idx": idx, "color": color, "depth": depth})
                if color == "red":
                    status_message = "Not center red/green"  # Adjust as needed based on position analysis

    if pins:
        red_pins = [pin for pin in pins if pin['color'] == 'red']
        if red_pins:
            min_depth_pin = min(pins, key=lambda x: x['depth'])
            if min_depth_pin in red_pins:
                status_message = "Strike for a FREE game!"

    # Add status message to the frame
    cv2.putText(frame, status_message, (frame.shape[1]//2 - 150, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
def main(media_folder, output_folder="processed_media"):
    ...
    depth_model_path = 'local::./DepthAnything/checkpoints/depth_anything_metric_depth_indoor.pt'
    global_settings = {
    ...
    model = YOLO("models/best_bowling_200epochs.pt")
    ...
    for media_path in media_files:
        ...
        if media_path.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            analyze_frame(img, model, depth_estimator, output_folder, original_name, debug=True)
        else:
            ...
            while cap.isOpened():
                ...
                if not ret:
                    break
                analyze_frame(frame, model, depth_estimator, output_folder, f"frame_{cap.get(cv2.CAP_PROP_POS_FRAMES):04d}.png", debug=True)
